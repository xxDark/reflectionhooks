...
final class JavaAccessNew implements JavaAccess {
	...
	public void init() throws Throwable {
		Environment.LOOKUP
				.findStatic(ReflectionFactory.class, "checkInitted",
	...
	static {
		// Once again, I hate Oracle
		// Try to bypass Jigsaw module system
		Lookup lookup = Environment.LOOKUP;
		try {
			...
			modules.addAll(
					(Set) layerModules
							.invoke(lookup.findStatic(layer, "boot", MethodType.methodType(layer))
									.invoke()));
			for (ClassLoader c = JavaAccessNew.class.getClassLoader(); c != null; c = c.getParent()) {
